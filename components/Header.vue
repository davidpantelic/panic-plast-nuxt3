<template>
  <a href="#main-content" class="skip-content-link">Preskoci do glavnog dela</a>
  <header>
    <nav :class="[isScrolling ? 'py-4' : 'py-5 bg-opacity-90']" class="bg-brand-black text-white fixed top-0 w-full flex justify-end px-6 lg:px-10 z-50">
      ...
      <ul class="nav-tabs items-center gap-5 hidden lg:flex mr-5">
        ...
        <li>
          <a href="/#kontakt" @click="focusKontakt" class="text-xl hover:text-brand-yellow">Kontakt</a>
        </li>
      </ul>
      ...
      <button :class="[{ 'collapsed': !isBurgerOpen }]" class="lg:hidden navbar-toggler rounded ml-5 flex flex-col items-center justify-center gap-[3px] border-white border-2 px-2 min-w-12 relative z-50" @click="toggleBurger" :aria-expanded="isBurgerOpen" aria-label="otvori i zatvori meni">
      ...
      <div class="overflow-hidden absolute top-full left-0 right-0 pointer-events-none">
        <ul :class="{ 'bg-opacity-90': !isScrolling, [!isBurgerOpen ? 'translate-y-[-100%]' : 'translate-y-0']: true }" class="nav-tabs flex flex-col lg:hidden px-6 pb-2 items-end gap-3 bg-brand-black transition-all pointer-events-auto" :aria-hidden="!isBurgerOpen">
          ...
          <li>
            <a href="/#kontakt" class="text-xl hover:text-brand-yellow" :tabindex="isBurgerOpen ? 0 : -1" @click="toggleBurger(); focusKontakt()">Kontakt</a>
          </li>
        </ul>
      </div>
    </nav>
